...
sc=s.create_script(open(r"G:\GiantsRE\scripts\frida_find_creator.js").read())
lines=[]
def on(m,d):
    p=m.get("payload") if m.get("type")=="send" else ("ERR "+str(m))
    lines.append(str(p))
sc.on("message",on); sc.load(); frida.resume(pid); time.sleep(12)
try: frida.kill(pid)
except: pass
os.makedirs(r"G:\GiantsRE\reports",exist_ok=True)
open(r"G:\GiantsRE\reports\frida_find_creator.txt","w").write("\n".join(lines))
